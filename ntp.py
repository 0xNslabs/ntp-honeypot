...
class SimpleNTPProtocol(DatagramProtocol):
    ...
    def datagramReceived(self, data, addr):
        ...
        try:
            ...
            leap_ver_mode, stratum, poll, precision, \
            root_delay, root_dispersion, ref_id, \
            ref_ts, orig_ts, recv_ts, trans_ts = unpacked
            ...
            log.msg(f"Leap: {leap}, Version: {version}, Mode: {mode}, Request Type: {request_type}, Stratum: {stratum}")

            if mode == 3:
                recv_timestamp = unpacked[8]
                tx_timestamp = self.system_time_to_ntp(time.time())
                response = pack('!B B b b I I 4s Q Q Q Q', 36, 1, 0, 0, 0, 0, b'LOCL', 0, recv_timestamp, tx_timestamp, tx_timestamp)
...
def main():
    ...
    parser.add_argument('--host', type=str, default='0.0.0.0', help='Host interface to bind the NTP server to.')
    parser.add_argument('--port', type=int, default=123, help='Port to bind the NTP server to.')
